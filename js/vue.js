$(function() {
	var GSHEET_JSON_URL = 'https://spreadsheets.google.com/feeds/list/1iNznWxmRchZLPqwf3zh2P18vjvIcpqL2JAJWsB6y5OU/1/public/values?alt=json';
	...
	var vueSearch = new Vue({
		...
		methods: {
			...
			updateResults: function() {
				...
				function(m, must, quoted, nonQuoted, rgxText, rgxOpts) {
					...
					searchTerms.push({
						...
						must: must == '+' ? true: must == '-' ? false: undefined
					});
				}
				);

				// Get the resulting list of links based on the scores.
				var results = self.links.reduce(function(scores, link, linkIndex) {
					var score = searchTerms.reduce(function(score, searchTerm) {
						if (score !== false) {
							...
							return searchTerm.must !== !matchesAny ? searchTerm.must !== false ? matchesTitle * 8 + matchesURL * 4 + matchesDescr * 2 + matchesKeywords * 1: 1: false;
						}
						return score;
					...
					return scores;
				}, []).sort(function(a, b) { return (b.score - a.score) || (b.index - a.index); }).map(function(score) { return score.link; });
			...
			markTerms: function(strIn, terms) {
				return terms.reduce(function(str, term) {
					return (YourJS.matchAll(strIn, term.rgx) || []).reduce(function(str, termMatch) {
						return str.replace(
							new RegExp('((?:[\\x01\\x02]*[^\\x01\\x02]){' + termMatch.index + '})((?:[\\x01\\x02]*[^\\x01\\x02]){' + (termMatch[0].length) + '})'),'$1\x01$2\x02'
						);
					}, str);
				}, strIn).replace(/\x01/g, '<div class="highlight">').replace(/\x02/g, '</div>');
			},
